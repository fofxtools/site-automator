...
class TestEnsureGitSafeDirectory:
    ...
    @patch("site_automator.wordops.SSHConnection")
    def test_configures_git_safe_directory_with_wildcard(self, mock_ssh_connection):
        """Test that git safe.directory is configured with wildcard."""
        mock_ssh_instance = Mock()
        mock_ssh_connection.return_value = mock_ssh_instance
        mock_ssh_instance.run_command.return_value = ("success", 0)

        wordops = WordOpsProvisioner(host="example.com")
        wordops.ensure_git_safe_directory()

        # Verify git config command was called with wildcard
        calls = mock_ssh_instance.run_command.call_args_list
        git_config_call = [
            call
            for call in calls
    ...
    @patch("site_automator.wordops.SSHConnection")
    def test_raises_on_git_config_failure(self, mock_ssh_connection):
        """Test that failure to configure git raises RuntimeError."""
        mock_ssh_instance = Mock()
        mock_ssh_connection.return_value = mock_ssh_instance
        mock_ssh_instance.run_command.side_effect = RuntimeError(
